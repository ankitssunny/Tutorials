...
public class CoinJamNew {
	...
	public void testcase() throws IOException{
		...
		Long maxToInt= (long) Integer.parseInt(max.toString());
		System.out.println("Case #" +count+ ":");
		while(j != 0){
			if(startsAndEndsWithOne() &&  findInterpretations() && (curr.length() <= n)){
			...
			Long currToInt=(long) (Integer.parseInt(curr.toString()) + 1);
			curr= toBinary(currToInt);
			while (currToInt < maxToInt && coinJam.contains(curr)){
				currToInt=(long) (Integer.parseInt(curr.toString()) + 1);
				curr= toBinary(currToInt);
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		CoinJamNew tests= new CoinJamNew();
